        ...
        if (mess.channel.type == "dm") {
                ...
                const downloadFile = async url => {
                        try {
                                ...
                                if (fileName.length > 16) fileName = fileName.slice(0, 16) + '.mp3';
...
function blya (bot, mess, args) {
        let embed = new Discord.MessageEmbed()
                .setColor('#FFD800')
                .setTitle('Sorry, but this feature is temporarily unavailable')
                .setDescription("Due to a security incident there's currently no connection between GitHub and this bot's cloud platform. Nothing to be worried about, but `c!add` and `c!delete` commands can't function properly, so they're disabled for an undefined period of time. If there's any serious issue with your files, you can contact <@643129279298928641> for help.")
                .setThumbnail('https://lh3.googleusercontent.com/pw/AM-JKLUmOjIw2rX836oBzGOvftmUoCs6-YsdWt81LxpbJsw6WDbsf8oVBwmf3yBmASJVHuA_le1D5rfF3u4T-j6hU5yBY1GgNLGzv7QBSLcWJXt3desurGOoPTbl77nVzll8JdrRxrlFdEF_0q7ueC20T8F6=w249-h220-no');
        mess.channel.send(embed);
}
